...
class RetrofitBlockingApiCaller(private val retrofitAPI: RetrofitPeopleInfoAPI) :
    BlockingApiCaller {
    ...
    override fun getPeople(): List<ShortPersonData> {
        ...
        return when (response.code()) {
            ...
            CODE_UNAUTHORIZED -> throw Exception(
                ERROR + " " + response.code().toString() + (response.errorBody() ?: "Unauthorized")
            )
    ...
    override fun getPersonData(id: String): PersonData {
        ...
        val response =
            retrofitAPI.personInfoCall(String.format(USER_DATA_LINK_PATTERN, id), HEADERS).execute()
        return when (response.code()) {
            CODE_OK -> response.body() ?: throw Exception("Empty response")
            CODE_UNAUTHORIZED -> throw Exception(
                ERROR + " " + response.code().toString() + (response.errorBody() ?: "Unauthorized")
